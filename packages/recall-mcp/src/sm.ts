...
export function loadConfig(): SmConfig {
  return {
...
async function smFetch(path: string, init: RequestInit = {}): Promise<Response> {
...
export async function smSearch(query: string, limit = 8): Promise<SearchHit[]> {
  const cfg = loadConfig();
...
export async function smAdd(content: string, customId?: string): Promise<{ id: string; status: string }> {
  const cfg = loadConfig();
...
export async function smList(limit = 20): Promise<Array<{ id: string; title?: string; summary?: string }>> {
  const cfg = loadConfig();
