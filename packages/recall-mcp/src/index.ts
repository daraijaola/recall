...
server.tool(
  "recall_search",
  "Search the user's portable RECALL memory (Supermemory Local hybrid search). Use before answering questions about the user's preferences, projects, or past decisions.",
  {
...
server.tool(
  "recall_context",
  "Get a packed context brief from RECALL (active preferences, decisions, constraints, project state). Call this when the user asks what they are working on or what you should know about them.",
  {
    query: z
      .string()
      .optional()
...
server.tool(
  "recall_remember",
  "Store a durable fact, preference, or decision into RECALL / Supermemory Local.",
  {
  ...
  async ({ content, customId }) => {
    try {
      ...
      if (!text) return errResult(new Error("content is empty"));
...
server.tool(
  "recall_forget",
  "Remove a memory from RECALL by Supermemory document id. Prefer an exact id from recall_search results.",
  {
  ...
  async ({ id }) => {
    try {
      const docId = id.trim();
      if (!docId) return errResult(new Error("id is empty"));
...
async function main() {
  ...
  console.error(
    `[recall-mcp] starting · sm=${cfg.url} · container=${cfg.container} · key=${cfg.apiKey ? "set" : "MISSING"}`,
  );
